-- ============================================
-- SWILL | FULL SCRIPT (FIXED NDS & MOBILE VIM + WALLCAM + HUD)
-- ============================================
...
local ndsEnabled = false -- НДС (Поиск в FOV + Имитация ЛКМ/Тача)
local aimFOV = 200
local drawFOV = false
local fovCircle = nil
local target = nil
...
local function isVisible(targetPart)
    ...
    if result then
        ...
        if model == targetPart.Parent then
            return true
        end
        return false
    end
...
RunService.RenderStepped:Connect(function()
    ...
    -- FPS (обновление каждые 0.5 секунд)
    if hudFPS then
...
local function simulateMouseClick(targetPart)
    local now = tick()
    if now - lastFire >= fireDelay then
        lastFire = now
        shotFiredTime = now
        if VIM and targetPart then
            local screenPos, onScreen = Camera:WorldToViewportPoint(targetPart.Position)
            if onScreen then
                pcall(function()
                    VIM:SendMouseButtonEvent(screenPos.X, screenPos.Y, 0, true, game, false)
                    task.wait()
                    VIM:SendMouseButtonEvent(screenPos.X, screenPos.Y, 0, false, game, false)
                end)
                pcall(function()
                    VIM:SendTouchEvent(1, Enum.UserInputState.Begin, screenPos.X, screenPos.Y, game)
                    task.wait()
                    VIM:SendTouchEvent(1, Enum.UserInputState.End, screenPos.X, screenPos.Y, game)
                end)
            end
        end
    end
...
CombatTab:CreateToggle({
    Name = "НДС (Имитация стрельбы по врагу в FOV)",
    CurrentValue = false,
    Flag = "NDSToggle",
    Callback = function(v) ndsEnabled = v end
})
...
ESPTab:CreateSection("HUD Информер (Экран)")
...
ESPTab:CreateToggle({
    Name = "Показывать FPS (раз в 0.5с)",
    CurrentValue = false,
    Flag = "HUDFPSToggle",
...
Rayfield:Notify({
    Title = "Готово",
    Content = "WallCam и HUD информер успешно добавлены!",
    Duration = 3,
    Image = 4483362458,
})