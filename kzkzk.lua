...
frame.Size = UDim2.fromOffset(200, 430)
...
makeButton("Trail", 233, function() return Settings.Trail end, function(v) 
    Settings.Trail = v
    trailEnabled = v
end)
makeButton("Spin", 266, function() return Settings.Spin end, function(v) 
    Settings.Spin = v
    updateSpin()
end)
makeButton("Zoom", 299, function() return Settings.Zoom end, function(v) 
    Settings.Zoom = v
    applyZoom()
end)
makeButton("Night", 332, function() return Settings.Night end, function(v) Settings.Night = v end)

-- Slider для Zoom
local zoomLabel = Instance.new("TextLabel")
zoomLabel.Size = UDim2.new(0.6, -5, 0, 25)
zoomLabel.Position = UDim2.new(0, 10, 0, 365)
...
local function makeSliderButton(text, x)
    ...
    btn.Position = UDim2.new(0, x, 0, 365)
...
makeSliderButton("R", 216).MouseButton1Click:Connect(function()
    DESIRED_ZOOM = 30
    zoomLabel.Text = "Zoom: " .. DESIRED_ZOOM
    if Settings.Zoom then applyZoom() end
end)
