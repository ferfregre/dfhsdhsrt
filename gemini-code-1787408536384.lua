-- ============================================
-- SWILL | FULL SCRIPT (NDS + TOUCH EMULATION FIX)
-- ============================================
...
local ndsEnabled = false -- НДС (Наводка + Симуляция тача)
local aimFOV = 200
local drawFOV = false
local fovCircle = nil
local target = nil
...
-- ============================
-- FOV КРУГ И НДС (ИМИТАЦИЯ ТАЧА ЧЕРЕЗ VIM)
-- ============================
...
local function simulateMouseClick()
    ...
    if now - lastFire >= fireDelay then
        ...
        if VIM then
            local center = Camera.ViewportSize / 2
            -- Имитируем нажатие пальцем (Touch) на мобилке для надежной стрельбы
            pcall(function()
                local touchId = 1
                VIM:SendTouchEvent(touchId, center.X, center.Y, true, game)
                task.wait()
                VIM:SendTouchEvent(touchId, center.X, center.Y, false, game)
            end)
        end
    end
end
...
-- НДС (Имитация тача)
CombatTab:CreateToggle({
    Name = "НДС (Имитация тача по врагу в FOV)",
    CurrentValue = false,
    Flag = "NDSToggle",
...
Rayfield:Notify({
    Title = "Готово",
    Content = "НДС с эмуляцией тача и Kill SFX загружен!",
    Duration = 3,
    Image = 4483362458,
})