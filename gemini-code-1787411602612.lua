-- ============================================
-- SWILL | FULL SCRIPT (FIXED NDS & MOBILE VIM + WALLCAM)
-- ============================================
...
-- WALLCAM (Оригинальная логика Wallcam для триггера)
...
-- ИСПРАВЛЕННАЯ ФУНКЦИЯ ВИДИМОСТИ (Игнорирует оружие и самого себя)
...
-- ============================
-- FOV КРУГ И НДС (ДВОЙНАЯ ИМИТАЦИЯ ЛКМ/ТАЧА)
-- ============================
...
local function simulateMouseClick()
    ...
    if now - lastFire >= fireDelay then
        ...
        if VIM then
            local center = Camera.ViewportSize / 2
            pcall(function()
                VIM:SendMouseButtonEvent(center.X, center.Y, 0, true, game, false)
                task.wait()
                VIM:SendMouseButtonEvent(center.X, center.Y, 0, false, game, false)
            end)
            pcall(function()
                VIM:SendTouchEvent(1, Enum.UserInputState.Begin, center.X, center.Y, game)
                task.wait()
                VIM:SendTouchEvent(1, Enum.UserInputState.End, center.X, center.Y, game)
            end)
        end
    end
end
...
-- ============================
-- WALLCAM (ИСПРАВЛЕННАЯ ВЕРСИЯ С АВТОПЕРЕКЛЮЧЕНИЕМ)
-- ============================
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Скрипт успешно объединен с WallCam хуком!",
    Duration = 3,
    Image = 4483362458,
})