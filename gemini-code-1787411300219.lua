...
-- ============================
-- WORLD & NIGHT CUSTOMIZATION
-- ============================
local function updateWorldVisuals()
    if Settings.PurpleWorld then
        Lighting.ClockTime = 0
        Lighting.Brightness = 0.5
        Lighting.Ambient = Color3.fromRGB(120, 50, 180)
        Lighting.OutdoorAmbient = Color3.fromRGB(90, 30, 140)
        Lighting.ColorShift_Top = Color3.fromRGB(180, 80, 255)
        Lighting.ColorShift_Bottom = Color3.fromRGB(100, 20, 160)
        return
    end
...
frame.Size = UDim2.fromOffset(200, 460)
frame.Position = UDim2.new(0.5, -100, 0.5, -230)
...
local function makeButton(text, y, get, set)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if text == "HP" then updateESP() end
        if text == "Night" or text == "PurpleWorld" then updateWorldVisuals() end
...
makeButton("ESP", 35, function() return Settings.ESP end, function(v) Settings.ESP = v end)
makeButton("HP", 68, function() return Settings.HP end, function(v) Settings.HP = v end)
makeButton("Ddd", 101, function() return Settings.Ddd end, function(v) Settings.Ddd = v dddEnabled = v end)
makeButton("Autofire", 134, function() return Settings.Autofire end, function(v) Settings.Autofire = v autofireEnabled = v end)
makeButton("JumpTrail", 167, function() return Settings.JumpTrail end, function(v) Settings.JumpTrail = v jumpTrailEnabled = v end)
makeButton("Bhop", 200, function() return Settings.Bhop end, function(v) Settings.Bhop = v end)
makeButton("Trail", 233, function() return Settings.Trail end, function(v) trailEnabled = v Settings.Trail = v end)
makeButton("Spin", 266, function() return Settings.Spin end, function(v) Settings.Spin = v updateSpin() end)
makeButton("Zoom", 299, function() return Settings.Zoom end, function(v) Settings.Zoom = v applyZoom() end)
makeButton("Night", 332, function() return Settings.Night end, function(v) Settings.Night = v updateWorldVisuals() end)
makeButton("PurpleWorld", 365, function() return Settings.PurpleWorld end, function(v) Settings.PurpleWorld = v updateWorldVisuals() end)

-- Slider для Zoom
local zoomLabel = Instance.new("TextLabel")
zoomLabel.Size = UDim2.new(0.6, -5, 0, 25)
zoomLabel.Position = UDim2.new(0, 10, 0, 398)
...
local function makeSliderButton(text, x)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.fromOffset(35, 25)
    btn.Position = UDim2.new(0, x, 0, 398)
...
makeSliderButton("-", 140).MouseButton1Click:Connect(function()
    DESIRED_ZOOM = math.max(5, DESIRED_ZOOM - 5)
    zoomLabel.Text = "Zoom: " .. DESIRED_ZOOM
    if Settings.Zoom then applyZoom() end
end)

makeSliderButton("+", 178).MouseButton1Click:Connect(function()
...
closeBtn.Position = UDim2.new(0.5, -30, 1, -28)
