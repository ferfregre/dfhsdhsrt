-- ============================================
-- SWILL | FULL SCRIPT (NDS + MOUSE CLICK FIX)
-- ============================================
...
local ndsEnabled = false -- НДС (Наводка + Симуляция ЛКМ)
local aimFOV = 200
local drawFOV = false
local fovCircle = nil
local target = nil
...
-- ============================
-- FOV КРУГ И НДС (ИМИТАЦИЯ ЛКМ ЧЕРЕЗ VIM)
-- ============================
...
local function simulateMouseClick()
    ...
    if now - lastFire >= fireDelay then
        ...
        if VIM then
            local center = Camera.ViewportSize / 2
            -- Имитируем зажатие и отпускание ЛКМ по центру экрана (0 - это левая кнопка мыши)
            pcall(function()
...
-- НДС (Имитация ЛКМ)
CombatTab:CreateToggle({
    Name = "НДС (Имитация ЛКМ по врагу в FOV)",
    CurrentValue = false,
    Flag = "NDSToggle",
...
Rayfield:Notify({
    Title = "Готово",
    Content = "НДС с имитацией ЛКМ и Kill SFX загружен!",
    Duration = 3,
    Image = 4483362458,
})